...
def Connnect_ipad_device ():
	...
	try:
		...
		import time
		desired_caps = {'platformName': 'Android', 'deviceName': '3cdbb8e5', 'platformVersion': '7.0',
			'sessionOverride': True, 'appPackage': 'com.yourenkeji.shenghuidai', 'newCommandTimeout': 600,
		# 一段时间不输入命令的话，app会退出。这个参数用来设置超时时间
			'appActivity': 'com.delevin.shenghuidai.welcome.WelcomeActivity', 'autoAcceptAlerts': True,
		# iOS 的个人信息访问警告(如位置'联系人、图片) 出现时，自动选择接受( Accept )。默认值 false。
...
def tongzhi ():
	...
	print (device.network_connection)
	print (device.available_ime_engines)
	device.activate_ime_engine ('com.sohu.inputmethod.sogou.zui/.SogouIME')  # 恢复默认输入法
	device.save_screenshot ("test.png")
