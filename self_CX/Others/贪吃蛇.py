...
class SnakeGame (Frame):
	def __init__ (self, master):
		...
		# 用于设置变色食物
		self.color_c = (
		"#FFB6C1", "#6A5ACD", "#0000FF", "#F0FFF0", "#FFFFE0", "#F0F8FF", "#EE82EE", "#000000", "#5FA8D9", "#32CD32")
		self.i = 0
		# 界面左侧显示分数
		self.m = StringVar ()
	...
	def key_release (self, event):
		...
		# 蛇不可以像自己的反方向走
		if key_dict.get (key) and not key == key_dict [self.snake.direction]:
	...
	def move (self, color = "#EE82EE"):
		...
		if self.snake.direction == 'Up':
			...
			new = ((head [0] + 1) % 24, head [1])  # 撞到自己，设置游戏结束的标志位，等待下一循环
